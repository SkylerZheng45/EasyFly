...
with open('/Users/abinavc/AA-Mock-Engine/mock/flightData.json') as f:
    ...
    for i in data.values():
    	...
    	for x in i["flights"]:
    		...
    		new_val = str(json.dumps(aircraftType))
    		cursor = connection.cursor()
    		sql_insert_query = cursor.execute("INSERT INTO flight_data (flight_number, aircraft_type, destination, flight_status, base_cost, duration_minutes, departuretime_hour, departuretime_minute) VALUES ( %s, %s, %s, %s, %s, %s, %s, %s)", (flightNumber, aircraftType, destination, flightStatus, baseCost, durationInMinutes, departureTime_hour, departureTime_minute));
